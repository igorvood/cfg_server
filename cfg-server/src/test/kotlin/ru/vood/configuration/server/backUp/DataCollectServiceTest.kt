...
import ru.vood.configuration.server.backUp.DataCollectService
import ru.vood.configuration.server.abstraction.AbstractDatasourceTests
import ru.vood.configuration.server.abstraction.compareTablesWithAsserts
...
internal class DataCollectServiceTest : AbstractDatasourceTests() {
    ...
    fun generateInsertsQueryOneTable() {
        ...
        assertEquals(
            """insert into test_1(default_val, id, num) 
select -1, 'id_1', 45 from dual  union 
select 15, 'id_2', 45 from dual """, generateInsertsQuery
        )
    }
