...
from_srv as (
  select DAGN.SERVICE_ID, DAGN.PROFILE_ID, DA.END_NODE_ID topic, 'prd' derrection, da.PROPERTY_KEY
  from srv DAGN
    join DICT_ARROW DA on DAGN.GRAPH_ID = DA.GRAPH_ID and DAGN.NODE_TYPE = DA.BEG_NODE_TYPE and DAGN.NODE_ID = DA.BEG_NODE_ID
),
to_srv as (
  select DAGN.SERVICE_ID, DAGN.PROFILE_ID, DA.BEG_NODE_ID topic, 'cns' derrection,  da.PROPERTY_KEY
  from srv DAGN
    join DICT_ARROW DA on DAGN.GRAPH_ID = DA.GRAPH_ID and DAGN.NODE_TYPE = DA.end_NODE_TYPE and DAGN.NODE_ID = DA.end_NODE_ID
     ),
all_topic as (
  select SERVICE_ID, PROFILE_ID, topic, derrection, PROPERTY_KEY
  from from_srv
  union all
  select SERVICE_ID, PROFILE_ID, topic, derrection, PROPERTY_KEY
  from to_srv
...
select
    ...
    al.derrection,
    tns.TOPIC_NAME,
    coalesce(tn.PRODUCER_PROP_GRP_REF, tn1.CONSUMER_PROP_GRP_REF) grp_prop,
    PROPERTY_KEY
from all_topic al
